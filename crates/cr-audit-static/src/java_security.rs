...
pub fn audit_mybatis_xml(xml_content: &str) -> Vec<Finding> {
...
pub fn audit_java_source(java_content: &str) -> Vec<Finding> {
    let mut findings = Vec::new();
...
pub fn audit_security(filename: &str, content: &str) -> Vec<Finding> {
    if filename.ends_with(".xml") && (content.contains("<mapper") || content.contains("DOCTYPE mapper")) {
        return audit_mybatis_xml(content);
    }
    if filename.ends_with(".java") {
        return audit_java_source(content);
    }
    Vec::new()
}
